...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($profilePicture)) {
        $updateQuery = "UPDATE client SET firstName = ?, email = ?, confirmPass = ?, password = ?, profile = ? WHERE id = ?";
        $stmt = $conn->prepare($updateQuery);
        $stmt->bind_param("sssssi", $firstname, $email, $confirmPassword, $password, $profilePicture, $clientID);
    } else {
        // Update name, email, password, and confirm_password in the database
        $updateQuery = "UPDATE client SET firstName = ?, email = ?, confirmPass = ?, password = ? WHERE id = ?";
        $stmt = $conn->prepare($updateQuery);
        $stmt->bind_param("ssssi", $firstname, $email, $confirmPassword, $password, $clientID);
    }
